...
export default function MapScreen({ navigation, route }) {
      ...
      cachedGet(`mapdata_${campusId}`, () => getMapData(campusId))
        .then((data) => {
          ...
          if (data.blocks?.length) {
            ...
            const bFloors = data.floors?.filter(
              (f) => f.blockId === data.blocks[0]._id,
            );
  ...
  const currentFloorRooms =
    mapData?.rooms?.filter((r) => r.floorId === selectedFloor?._id) || [];
  const currentFloorNodes =
    mapData?.nodes?.filter((n) => n.floorId === selectedFloor?._id) || [];
  const currentFloorPaths =
    mapData?.paths?.filter((p) => p.floorId === selectedFloor?._id) || [];
  const blockFloors =
    mapData?.floors?.filter((f) => f.blockId === selectedBlock?._id) || [];
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <Text style={{ color: colors.textSec, marginTop: 12 }}>
          Loading map...
        </Text>
      </View>
    );
    ...
    <View style={s.container}>
      ...
      <View style={s.topBar}>
        {mapData?.blocks?.map((block) => (
          <TouchableOpacity
            ...
            style={[
              s.blockBtn,
              selectedBlock?._id === block._id && s.blockBtnActive,
            ]}
            onPress={() => {
              setSelectedBlock(block);
              const bFloors = mapData.floors?.filter(
                (f) => f.blockId === block._id,
              );
          ...
          >
            <Text
              style={[
                s.blockBtnText,
                selectedBlock?._id === block._id && s.blockBtnTextActive,
              ]}
            >
              {block.name}
            </Text>
      ...
      <View style={s.floorBar}>
        {blockFloors.map((floor) => (
          <TouchableOpacity
            ...
            style={[
              s.floorBtn,
              selectedFloor?._id === floor._id && s.floorBtnActive,
            ]}
            onPress={() => {
          ...
          >
            <Text
              style={[
                s.floorBtnText,
                selectedFloor?._id === floor._id && s.floorBtnTextActive,
              ]}
            >
              {floor.name}
            </Text>
      ...
      <View style={s.mapArea}>
        <Svg
          width={SW}
          height={SH - 200}
          viewBox={`${-panOffset.x} ${-panOffset.y} ${SW / zoom} ${(SH - 200) / zoom}`}
        >
          {/* Grid */}
          ...
          {currentFloorRooms.map((room) => {
            ...
            const fillColor =
              (COLORS[room.type] || "#3b82f6") + (isSelected ? "80" : "30");
            const strokeColor = isSelected
              ? "#818cf8"
              : COLORS[room.type] || "#3b82f6";
        ...
        <View style={s.controls}>
          ...
          <TouchableOpacity
            style={s.ctrlBtn}
            onPress={() => setZoom((z) => Math.max(0.2, z - 0.2))}
          >
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              {selectedRoom.roomNumber ? (
                <Text
                  style={{ fontSize: 13, color: colors.textSec, marginTop: 2 }}
                >
