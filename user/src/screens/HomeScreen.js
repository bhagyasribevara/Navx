    ...
    <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.quickGrid}>
        {quickActions.map((action, i) => (
          ...
          >
            <View
              style={[s.quickIcon, { backgroundColor: action.color + "20" }]}
            >
      ...
      <View style={s.section}>
        ...
        {campuses.length === 0 ? (
          <Text style={s.emptyText}>
            No campuses available.{"\n"}Ask admin to set up your campus.
          </Text>
        ) : (
          campuses.map((campus) => (
            <TouchableOpacity
            ...
            >
              ...
              <Text style={s.campusDesc}>
                {campus.description || "Tap to explore"}
              </Text>
              {campus.address ? (
                <Text style={[s.campusDesc, { fontSize: 12 }]}>
                  📍 {campus.address}
                </Text>
              ) : null}
              <TouchableOpacity
                style={s.campusBtn}
                onPress={() =>
                  navigation.navigate("Map", { campusId: campus._id })
                }
              >
