...
export default function SettingsScreen() {
  const { colors, isDark, setIsDark, language, setLanguage } =
    ...
    >
      ...
      >
        <Ionicons
          name={icon}
          size={18}
          color={iconBg ? "#fff" : colors.primary}
        />
      </View>
  ...
  return (
    <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
      ...
      <View
        style={[s.row, s.rowFirst, s.rowLast, { flexWrap: "wrap", gap: 0 }]}
      >
        {LANGUAGES.map((lang) => (
          <TouchableOpacity
            key={lang.code}
            style={[
              s.langBtn,
              language === lang.code ? s.langBtnActive : s.langBtnInactive,
            ]}
          ...
          >
            <Text
              style={[
                s.langBtnText,
                { color: language === lang.code ? "#fff" : colors.textSec },
              ]}
            >
              {lang.label}
            </Text>
