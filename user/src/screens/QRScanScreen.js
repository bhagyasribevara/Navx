  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          { text: 'Navigate', onPress: () => navigation.navigate('Map', { userPosition: qrData.position, floorId: qrData.floorId?._id }) },
          { text: 'Scan Again', onPress: () => setScanned(false) },
        ]
      );
    } catch (err) {
      Alert.alert('QR Not Found', 'This QR code is not registered in the navigation system.', [
        { text: 'Scan Again', onPress: () => setScanned(false) },
  ...
  if (!permission?.granted) {
    return (
      <View style={[s.center, { backgroundColor: colors.bg }]}>
        ...
        <Text style={[s.title, { color: colors.text }]}>Camera Access Required</Text>
      ...
      <View style={s.overlay}>
        ...
        <View style={s.scanArea}>
          ...
          <Text style={s.scanHint}>
            {scanned ? '✅ QR Code detected!' : 'Point camera at a NavX QR code'}
          </Text>
        </View>
        ...
        <View style={s.bottomInfo}>
          <View style={s.infoCard}>
            ...
            <Text style={s.infoText}>
              QR codes are placed at entrances, corridors, and key locations to set your indoor position
            </Text>
          </View>
          {scanned && (
...
const s = StyleSheet.create({
  ...
  desc: { fontSize: 14, textAlign: 'center', marginTop: 8, paddingHorizontal: 30, lineHeight: 20 },
  permBtn: { marginTop: 24, paddingHorizontal: 32, paddingVertical: 14, borderRadius: 14 },
  permBtnText: { color: '#fff', fontWeight: '700', fontSize: 15 },
  overlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'space-between' },
  topBar: { flexDirection: 'row', alignItems: 'center', paddingTop: 50, paddingHorizontal: 16, paddingBottom: 12 },
  ...
  scanHint: { color: '#fff', fontSize: 14, fontWeight: '500', marginTop: 20, textShadowColor: 'rgba(0,0,0,0.8)', textShadowRadius: 4, textShadowOffset: { width: 0, height: 1 } },
  bottomInfo: { padding: 20, paddingBottom: 40 },
  infoCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(26,32,53,0.9)', padding: 14, borderRadius: 14, gap: 10 },
  infoText: { flex: 1, color: '#94a3b8', fontSize: 13, lineHeight: 18 },
  rescanBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(99,102,241,0.15)', paddingVertical: 12, borderRadius: 12, marginTop: 10, gap: 8, borderWidth: 1, borderColor: 'rgba(99,102,241,0.3)' },
