...
import Svg, { Rect, Circle, Line, G, Text as SvgText, Polygon } from 'react-native-svg';
...
export default function NavigationScreen({ navigation, route }) {
  ...
  useEffect(() => {
    if (routeData && userPos && isNavigating) {
      ...
      if (currentTarget) {
        const dist = Math.hypot(userPos.x - currentTarget.x, userPos.y - currentTarget.y);
        if (dist < 20) {
          // Reached waypoint
  ...
  const floorNodes = mapData?.nodes?.filter(n => n.floorId === room?.floorId) || [];
  ...
  const s = StyleSheet.create({
    ...
    directionCard: { position: 'absolute', top: 12, left: 12, right: 12, backgroundColor: colors.card, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: colors.primary + '40', flexDirection: 'row', alignItems: 'center' },
    dirIcon: { width: 50, height: 50, borderRadius: 14, backgroundColor: colors.primary + '20', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
    dirText: { fontSize: 16, fontWeight: '700', color: colors.text },
    dirDist: { fontSize: 13, color: colors.textSec, marginTop: 2 },
    bottomInfo: { backgroundColor: colors.card, padding: 16, borderTopWidth: 1, borderTopColor: colors.border },
    ...
    startBtn: { backgroundColor: colors.primary, paddingVertical: 14, borderRadius: 14, alignItems: 'center', flexDirection: 'row', justifyContent: 'center' },
    startBtnText: { fontSize: 16, fontWeight: '700', color: '#fff', marginLeft: 8 },
    arBtn: { backgroundColor: colors.surface, paddingVertical: 12, borderRadius: 12, alignItems: 'center', marginTop: 8, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', justifyContent: 'center' },
  });
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        ...
        <Text style={s.title} numberOfLines={1}>Navigate to {room?.name || 'Destination'}</Text>
        <TouchableOpacity onPress={() => setVoiceEnabled(!voiceEnabled)}>
          <Ionicons name={voiceEnabled ? 'volume-high' : 'volume-mute'} size={22} color={voiceEnabled ? colors.primary : colors.textMuted} />
        </TouchableOpacity>
      </View>
      ...
      <View style={s.mapArea}>
        <Svg width={SW} height={SH - 320} viewBox="0 0 800 600">
              ...
              <G key={r._id}>
                {sh.type === 'circle' ? (
                  <Circle cx={sh.x + (sh.radius || 30)} cy={sh.y + (sh.radius || 30)} r={sh.radius || 30}
                    fill={isTarget ? '#6366f160' : '#3b82f620'} stroke={isTarget ? '#818cf8' : '#3b82f6'} strokeWidth={isTarget ? 3 : 1} />
                ) : (
          ...
          {routeData?.path?.map((p, i) => {
            ...
            return <Line key={`rp-${i}`} x1={prev.x} y1={prev.y} x2={p.x} y2={p.y}
              stroke={isPast ? '#22c55e' : '#6366f1'} strokeWidth={4} strokeLinecap="round" />;
          })}
          ...
          {userPos && (
            <G>
              ...
              <Circle cx={userPos.x} cy={userPos.y} r={7} fill="#6366f1" stroke="#fff" strokeWidth={2} />
            </G>
          )}
          ...
          {room?.shape && (
            <G>
              <Circle cx={room.shape.type === 'circle' ? room.shape.x + (room.shape.radius || 30) : room.shape.x + (room.shape.width || 80) / 2}
                cy={room.shape.type === 'circle' ? room.shape.y - 10 : room.shape.y - 10}
        ...
        {isNavigating && currentDirection && (
          <View style={s.directionCard}>
            <View style={s.dirIcon}>
              <Ionicons name={currentDirection.instruction.includes('left') ? 'arrow-back' : currentDirection.instruction.includes('right') ? 'arrow-forward' : 'arrow-up'} size={24} color={colors.primary} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={s.dirText}>{currentDirection.instruction}</Text>
              <Text style={s.dirDist}>{currentDirection.distance}px · Step {currentStep + 1}/{routeData.directions.length}</Text>
            </View>
          </View>
        )}
      </View>
      ...
      <View style={s.bottomInfo}>
        ...
        {!isNavigating ? (
          ...
          <TouchableOpacity style={[s.startBtn, { backgroundColor: colors.danger }]} onPress={() => { setIsNavigating(false); Speech.stop(); }}>
            <Ionicons name="stop" size={20} color="#fff" />
            <Text style={s.startBtnText}>Stop Navigation</Text>
          </TouchableOpacity>
        )}

        <TouchableOpacity style={s.arBtn} onPress={() => navigation.navigate('AR', { routeData, room, heading })}>
          <Ionicons name="camera" size={18} color={colors.primary} />
          <Text style={{ color: colors.primary, fontWeight: '600', marginLeft: 8 }}>Switch to AR View</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
