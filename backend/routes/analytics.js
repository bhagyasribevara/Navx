...
router.get('/summary/:campusId', async (req, res) => {
  try {
    ...
    const [navCount, searchCount, qrCount, topSearches, topRoutes] = await Promise.all([
      ...
      Analytics.aggregate([
        { $match: { campusId: require('mongoose').Types.ObjectId(campusId), type: 'search', timestamp: { $gte: since } } },
        { $group: { _id: '$data.searchQuery', count: { $sum: 1 } } },
        { $sort: { count: -1 } }, { $limit: 10 }
      ]),
      Analytics.aggregate([
        { $match: { campusId: require('mongoose').Types.ObjectId(campusId), type: 'navigation', timestamp: { $gte: since } } },
...
// GET heatmap data
router.get('/heatmap/:campusId', async (req, res) => {
  try {
    const { campusId } = req.params;
